...
import social.entourage.android.old_v7.base.location.LocationListener_v7
import social.entourage.android.old_v7.base.location.LocationProvider_v7
 ...
 * @see EntService_v7
 */
open class EntServiceManager_v7(
    val entService: EntService_v7,
    ...
    val locationProvider: LocationProvider_v7
) {
    ...
    // INNER CLASSES
    // ----------------------------------
    internal class NewsFeedCallback(private val manager: EntServiceManager_v7, private val service: EntService_v7) : Callback<NewsfeedItemResponse?> {
    ...
    internal class HomeFeedCallback(private val manager: EntServiceManager_v7, private val service: EntService_v7) : Callback<ResponseBody?> {
    ...
    companion object {
        ...
        // CONSTANTS
        // ----------------------------------
        fun newInstance(entService: EntService_v7,
                        authenticationController: AuthenticationController,
                        newsfeedRequest: NewsfeedRequest,
                        entourageRequest: EntourageRequest): EntServiceManager_v7 {
            val provider = LocationProvider_v7(entService)
            val mgr = EntServiceManager_v7(
                    entService,
                    authenticationController,
                    newsfeedRequest,
                    entourageRequest,
                    provider)
            provider.locationListener = LocationListener_v7(mgr, entService)
            provider.start()
            EntBus.register(mgr)
            return mgr
        }
