...
open class FeedItemViewHolder(itemView: View) : BaseCardViewHolder(itemView) {
    ...
    override fun bindFields() {
        ...
        itemView.action_card_button_act?.setOnClickListener { onClickCardButton() }
    }
    ...
    override fun populate(data: TimestampedObject) {
        ...
        if (!feedItem.showAuthor()) {
            itemView.action_card_author?.text = ""
            itemView.action_card_photo?.setImageDrawable(null)
            itemView.action_card_partner_logo?.setImageDrawable(null)
        }
        ...
        //Feed Item type
        itemView.action_card_type?.text = feedItem.getFeedTypeLong(itemView.context)
        if (feedItem.getFeedTypeColor() != 0) {
            itemView.action_card_type?.setTextColor(ContextCompat.getColor(itemView.context, feedItem.getFeedTypeColor()))
        }

        if ((feedItem as? BaseEntourage)?.isOnlineEvent == true) {
            itemView.action_card_location?.text = res.getString(R.string.info_event_online_feed)
        } else {
            ...
            itemView.action_card_location?.text = distStr
        }

        //entourage members
        itemView.action_card_people_count?.text = res.getString(R.string.entourage_cell_numberOfPeople, feedItem.numberOfPeople)

        //badge count
        if (feedItem.getUnreadMsgNb() <= 0) {
            itemView.action_card_badge_count?.visibility = View.GONE
        } else {
            itemView.action_card_badge_count?.visibility = View.VISIBLE
            itemView.action_card_badge_count?.text = res.getString(R.string.badge_count_format, feedItem.getUnreadMsgNb())
        }

        //act button
        if (itemView.action_card_button_act != null) {
            //var dividerColor = R.color.accent
            var textColor = R.color.accent
            itemView.action_card_button_act?.visibility = View.VISIBLE
            if (feedItem.isClosed()) {
                itemView.action_card_button_act?.setText(feedItem.getClosedCTAText())
                //dividerColor = R.color.greyish
                textColor = feedItem.getClosedCTAColor()
            } else {
                when (feedItem.joinStatus) {
                    FeedItem.JOIN_STATUS_PENDING -> {
                        itemView.action_card_button_act?.setText(R.string.entourage_cell_button_pending)
                    }
                    FeedItem.JOIN_STATUS_ACCEPTED -> {
                        EntourageApplication.get().me()?.let { currentUser ->
                            if (author?.userID == currentUser.id) {
                                itemView.action_card_button_act?.setText(R.string.entourage_cell_button_accepted)
                            } else {
                                itemView.action_card_button_act?.setText(R.string.entourage_cell_button_accepted_other)
                            }
                        } ?: run {
                            itemView.action_card_button_act?.setText(R.string.entourage_cell_button_accepted)
                        }
                    ...
                    FeedItem.JOIN_STATUS_REJECTED -> {
                        itemView.action_card_button_act?.setText(R.string.entourage_cell_button_rejected)
                        textColor = R.color.tomato
                    }
                    else -> {
                        itemView.action_card_button_act?.setText(R.string.entourage_cell_button_view)
                        //dividerColor = R.color.greyish
                        itemView.action_card_button_act?.visibility = View.INVISIBLE
                    }
                }
            }
            itemView.action_card_button_act?.setTextColor(ContextCompat.getColor(itemView.context, textColor))
        }
