        ...
        with(holder) {
            when (getItemViewType(position)) {
                ...
                TYPE_SURVEY -> {
                    ...
                    surveyHolder.binding.name.text =
                        post.user?.displayName ?: "Nom inconnu"
                    Glide.with(context)
                    ...
                    survey.choices.forEachIndexed { index, choice ->
                        ...
                        bindingChoice?.let {
                            ...
                            val progress = if (totalResponses > 0)
                                (choiceResponses * 100 / totalResponses) else 0
                            ...
                            it.parent.setOnClickListener {
                                ...
                                if (survey.multiple) {
                                    ...
                                    if (localState[index]) {
                                        ...
                                        localSummary[index] =
                                            (localSummary.getOrNull(index)?.minus(1))
                                                ?.coerceAtLeast(0) ?: 0
                                    }
                                } else {
                                    ...
                                    if (previouslySelectedIndex != -1) {
                                        localSummary[previouslySelectedIndex] =
                                            (localSummary[previouslySelectedIndex] - 1)
                                                .coerceAtLeast(0)
                                ...
                                if (postsList[position].commentsCount != null) {
                                    ...
                                    else R.string.posts_vote_number_singular
                                    val commentsText = " - " + String.format(
                                        holder.itemView.context.getString(resId),
                                        totalResponses
                                    )
                    ...
                    if (postsList[position].status in listOf("deleted", "offensive", "offensible")) {
                        surveyHolder.binding.surveyQuestion.text =
                            context.getText(R.string.deleted_publi)
                        if (postsList[position].status in listOf("offensive", "offensible")) {
                            surveyHolder.binding.surveyQuestion.text =
                    ...
                    with(postsList[position]) {
                        ...
                        reactions?.forEachIndexed { index, reaction ->
                            if (index < reactionsLayouts.size) {
                                val reactionType =
                                    reactionTypes?.find { it.id == reaction.reactionId }
                                reactionType?.let {
                                    Glide.with(context)
                                        .load(it.imageUrl)
                        ...
                        if (hasComments == false) {
                            ...
                            if (commentsCount != null) {
                                val resId =
                                    if (commentsCount > 1) R.string.posts_comment_number
                                    else R.string.posts_comment_number_singular
                                val commentsText = String.format(
                                    holder.itemView.context.getString(resId),
                                    commentsCount
                                )
                        ...
                        if (survey.summary.isEmpty()) {
                            ...
                            if (commentsCount != null && commentsCount != 0) {
                                val resId =
                                    if (totalResponses > 1) R.string.posts_vote_number
                                    else R.string.posts_vote_number_singular
                                val commentsText = " - " + String.format(
                                    holder.itemView.context.getString(resId),
                                    totalResponses
                ...
                TYPE_POST -> {
                    ...
                    with(postsList[position]) {
                        ...
                        if (DataLanguageStock.userLanguage == this.contentTranslations?.fromLang
                            || (this.user?.id == meId?.toLong()) && (this.content == "")
                        ) {
                        ...
                        if (contentTranslations != null) {
                            contentToShow =
                                if (isTranslated) contentTranslations.translation
                                else contentTranslations.original
                        }

                        // Vérifie s'il y a déjà une balise <a href="...">
                        //val hasLink = contentToShow?.contains("<a href=") == true
                        val hasLink = true
                        if (hasLink) {
                            // On parse le HTML
                            ...
                            // Sinon, texte brut + setHyperlinkClickable
                        ...
                        if (status in listOf("deleted", "offensive", "offensible")) {
                            ...
                            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                                binding.postMessage.setTextColor(
                        ...
                        // Désactiver les clics sur réactions si pas membre
                        if (isMember == false) {
                            binding.btnReportPost.setOnClickListener {
    ...
    fun adjustSurveyTotalsBasedOnLocalState(
        ...
        localState.forEachIndexed { index, isSelected ->
            ...
            if (isSelected && !wasSelected) {
                ...
                adjustedSummary[index] =
                    (adjustedSummary[index] - 1).coerceAtLeast(0)
            }
        }
    ...
    private fun handleReactionClick(post: Post, reactionType: ReactionType) {
        ...
        if (currentUserReactionId != 0) {
            val existingReaction =
                post.reactions?.find { it.reactionId == currentUserReactionId }
